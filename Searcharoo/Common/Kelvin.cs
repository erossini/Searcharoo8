...
namespace cd.net
{
    public static class Kelvin<T> // where T : ISerializable // prevents string[] from being Serialized
    {
        ...
        /// <summary>
        /// Load Xml from a Url and deserialize
        /// </summary>
        public static T FromXmlResponse(string frozenObjectXmlUrl)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(T));
            System.Net.WebClient client = new System.Net.WebClient();
            using (System.IO.Stream stream = client.OpenRead(frozenObjectXmlUrl))
            {
        ...
        /// <summary>
        /// Load Xml stream from the given assembly/resource-name and deserialize
        /// </summary>
        /// <param name="a">Assembly object</param>
        /// <param name="frozenObjectResourceName">Resource filename</param>
        public static T FromResource(System.Reflection.Assembly a, string frozenObjectResourceName)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(T));
            System.Net.WebClient client = new System.Net.WebClient();
            using (System.IO.Stream stream = a.GetManifestResourceStream(frozenObjectResourceName))
            {
                object o = serializer.Deserialize(stream);
                stream.Close();
                return (T)o;
            ...
            if (System.IO.File.Exists(frozenObjectFileName))
            {
                ...
                {   // "Unable to find assembly 'App_Code.vj-e_8q4, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null'."
                    ...
                    try
                    {
                        ...
                        return (T)formatter.Deserialize(stream); // // Try the custom BasicBinder to see if that helps us 'find' the assembly
                    }
                    catch
                    {
                        throw;
